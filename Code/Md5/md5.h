...
// the MD5 context.
  MD5 (unsigned char *string) // digest string, finalize
  {
		...
		update (string, (unsigned int)strlen((char*)string));
		finalize();
  }
...
// methods to acquire finalized result
  ...
  char *            hex_digest (char* s)  // digest as a 33-byte ascii-hex string
  {
		int i;
  ...
  char *            hex_digest_lower (char* s)  // digest as a 33-byte ascii-hex string lower
  {
		int i;
...
// last, the private methods, mostly static:
  ...
  void encode    (uint1 *output, uint4 *input, uint4 length)
  {
		unsigned int i, j;
  ...
  void decode    (uint4 *output, uint1 *input, uint4 length)
  {
		unsigned int i, j;
  ...
  void memcpy    (uint1 *output, uint1 *input, uint4 length)
  {
		unsigned int i;
  ...
  void memset    (uint1 *output, uint1 val, uint4 length)
  {
		unsigned int i;
...
inline std::ostream& operator << (std::ostream &stream, MD5 context){

  stream << context.hex_digest();
  return stream;
}

inline const unsigned char* GetMD5(const char* password)
{
	MD5 md5;
	md5.init();
	md5.update((unsigned char*)password,(unsigned int)strlen(password));
	md5.finalize();
	return md5._digest();
}

inline std::string GetMD5Str(const std::string& str)
{
	MD5 md5;
	md5.init();
	md5.update((unsigned char*)str.data(),(unsigned int)str.size());
	md5.finalize();
	char buffer[64];
...
inline std::string GetMD5StrLower(const std::string& str)
{
	MD5 md5;
	md5.init();
	md5.update((unsigned char*)str.data(),(unsigned int)str.size());
