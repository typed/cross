...
void cRakNetCSServer::RunOnce()
{
	...
	CASE_PACKAGE_BEGIN(ID_TIMESTAMP)
		...
		//			//LogError("ʹ�ü�����") << m_pCallback->GetPlayerName(it->second->m_id) << " sumtime:" << t << "ms ipport:" << pPacket->systemAddress.ToString() << LEnd;
		//		}
		//		else {
		//			m_pPeer->CloseConnection(pPacket->guid, true);
		//			//LogError("ʹ�ü�����") << " sumtime:" << t << "ms ipport:" << pPacket->systemAddress.ToString() << LEnd;
		//		}
		//	}
		//}
